...
resource "local_file" "config" {
  content = templatefile("${path.module}/files/default.template.conf", {
    OAUTH_CLIENT_ID      = var.config.oauth_client_id
    AUDIENCE             = var.config.audience
    UPSTREAM_URL         = var.config.upstream_url
    AUTHORIZED_DOMAIN    = var.config.authorized_domain
...
resource "google_project_iam_member" "pubsub_token_creator" {
  ...
  member  = "serviceAccount:service-${var.config.project_number}@gcp-sa-pubsub.iam.gserviceaccount.com"
}
...
resource "google_pubsub_subscription" "httpwal" {
  ...
  push_config {
    push_endpoint = "${var.config.service_url}/wal-playback/"
